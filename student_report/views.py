...
def charts(request,id):
    return render(request, "student_report/charts.html", {"sid":id})
...
def tables(request,id):
    ...
    for i in range(len(student_courses)):
        ...
        all_course_table.append([student_courses[i].course,student_courses[i].overall,c_average,student_courses[i].class_rank])
    this_course_table = []
    user = User.objects.get(username=request.user)
    profile = professor_profile.objects.get(professor=user)
    ...
    for i in range(len(quizzes)):
        ...
        exam_marks = Marks.objects.get(student_id=id,prof_id=user,course_id=profile.professor_course,q_name=quizzes[i].quiz_name)
        this_course_table.append([quizzes[i].quiz_name.split('-')[1],quizzes[i].avg_marks,quizzes[i].max_marks,exam_marks.marks])
    return render(request, "student_report/tables.html",{'sid':id,'course_table':all_course_table,'this_course':this_course_table})
